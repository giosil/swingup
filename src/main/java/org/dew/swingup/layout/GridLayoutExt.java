...
/**
 * The <code>GridLayoutExt</code> class e' una replica di GridLayout che consente lo span di una colonna.
 *
 * @author <a href="mailto:giorgio.giorgio.silvestris@gmail.com">Giorgio Silvestris</a>
 * @version 1.0
 */
public class GridLayoutExt implements LayoutManager, java.io.Serializable {
    /*
     * serialVersionUID
     */
    private static final long serialVersionUID = -3490801692683745245L;
    protected int __cols = -1;
    protected int __span =  1;
    protected int __diff =  0;
    public void setColSpan(int columnToSpan, int span) {
        this.__cols = columnToSpan;
        this.__span = span;
        if(__span < 1) __span = 1;
        __diff = __span - 1;
    }
    public void setColSpan(int[] aiColAndSpan) {
        if(aiColAndSpan == null || aiColAndSpan.length < 2) return;
        this.__cols = aiColAndSpan[0];
        this.__span = aiColAndSpan[1];
        if(__span < 1) __span = 1;
        __diff = __span - 1;
    }
    public GridLayoutExt(int rows, int cols, int hgap, int vgap, int columnToSpan, int span) {
        if ((rows == 0) && (cols == 0)) {
     ...
     */
    public GridLayoutExt(int rows, int cols) {
        this(rows, cols, 0, 0);
    }
     ...
     *                  set to zero
     */
    public GridLayoutExt(int rows, int cols, int hgap, int vgap) {
        if ((rows == 0) && (cols == 0)) {
            throw new IllegalArgumentException("rows and cols cannot both be zero");
        }
        this.rows = rows;
    ...
    public void layoutContainer(Container parent) {
      synchronized (parent.getTreeLock()) {
        ...
        int totalGapsWidth = (ncols + __diff - 1) * hgap; // Ext
        int widthWOInsets = parent.getWidth() - (insets.left + insets.right);
        int widthOnComponent = (widthWOInsets - totalGapsWidth) / (ncols + __diff); // Ext
        int extraWidthAvailable = (widthWOInsets - (widthOnComponent * (ncols + __diff) + totalGapsWidth)) / 2; // Ext
        ...
        if (ltr) {
            for (int c = 0, x = insets.left + extraWidthAvailable; c < ncols ; c++, x += widthOnComponent + hgap) {
                for (int r = 0, y = insets.top + extraHeightAvailable; r < nrows ; r++, y += heightOnComponent + vgap) {
                    ...
                    if (i < ncomponents) {
                        if(c == __cols) { // Ext
                            parent.getComponent(i).setBounds(x, y, widthOnComponent * __span + hgap * __diff, heightOnComponent); // Ext
                            x += (widthOnComponent + hgap) * __diff; // Ext
                        }
                        else {
